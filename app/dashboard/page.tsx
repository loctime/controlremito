...
function DashboardContent() {
  ...
  const [expandedInTransitOrder, setExpandedInTransitOrder] = useState<string | null>(null)
  ...
  const toggleInTransitOrderExpansion = (orderId: string) => {
    if (expandedInTransitOrder === orderId) {
      setExpandedInTransitOrder(null)
      setOrderDetails("")
      setItemQuantities({})
    } else {
      setExpandedInTransitOrder(orderId)
      setOrderDetails("")
      setItemQuantities({})
    }
  }
  ...
  const markOrderAsReceived = async (orderId: string) => {
    ...
    try {
      // Actualizar el pedido como recibido
                                    ...
                                    <td className="py-3 px-2">
                                      <div className="flex justify-center">
                                        <Button
                                          size="sm"
                                          variant="outline"
                                          onClick={() => toggleInTransitOrderExpansion(order.id)}
                                          className="text-xs px-3 py-1 h-auto"
                                        >
                                          {expandedInTransitOrder === order.id ? (
                                            <>
